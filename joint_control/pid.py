...
class PIDController(object):
    ...
    def control(self, target, sensor):
        '''apply PID control
        @param target: reference values w    e = w-y
        @param sensor: current values from sensor Y
        @return control signal
        '''
        # YOUR CODE HERE
        e = target - sensor
        self.u = self.u + (self.Kp + self.Ki*self.dt + (self.Kd / self.dt))* e - (self.Kp + (self.Kd * 2)/self.dt)* e + e * (self.Kd/self.dt)
